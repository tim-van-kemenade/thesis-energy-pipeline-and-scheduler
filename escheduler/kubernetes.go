// Copyright 2016 Google Inc and modifications by Tim van Kemenade to use service-account authentication. All Rights Reserved.
...
var (
	apiHost           = "192.168.221.2:6443" // "127.0.0.1:6443"
...
func fit(pod *Pod) ([]Node, error) {
	...
	if len(fitFailures) > 0 {
		fmt.Printf("\nFailed to fit on some nodes. Only %d nodes available. Failures:\n%s\n", len(nodes), strings.Join(fitFailures, "\n"))
	}
	...
	if len(nodes) == 0 {
		...
		fmt.Printf("\nFAILED to schedule pod '%s'  at %v \n", pod.Metadata.Name, timestamp)
		...
		event := Event{
			...
			Type:           "Warning",
			Source:         EventSource{Component: "energy-scheduler"},
...
func bind(pod *Pod, node Node) error {
	...
	if resp.StatusCode != 201 {
		fmt.Printf("Binding error: expected a 201, but received HTTP status code %s", resp.Status)
